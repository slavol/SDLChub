...
export default function ReportsPage() {
  ...
  const sprintBurnup = report?.sprint_burnup || [];
  const cumulativeFlow = report?.cumulative_flow || [];
  const wipHistory = report?.wip_history || [];
  const wipLimits = report?.wip_limits || [];
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-4 py-5 text-slate-100 sm:px-6 lg:px-10">
      ...
      <div className="mb-6 grid min-w-0 gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
        ...
        />
        <MetricCard
          title="Flow lead time"
          value={`${metrics.average_flow_lead_time_days}d`}
          subtitle={`${metrics.flow_completed_tasks} Kanban/flow tasks completed`}
          icon={Timer}
        />
        <MetricCard
          title="WIP alerts"
          value={metrics.wip_limit_violations}
          subtitle="Workflow columns currently above WIP limits"
          icon={Gauge}
          danger={metrics.wip_limit_violations > 0}
        />
        <MetricCard
          title="Overdue"
      ...
      <div className="grid min-w-0 gap-6 xl:grid-cols-2">
        ...
        <ChartCard
          title="Active sprint scope split"
        ...
        <ChartCard
          title="Sprint burnup"
          description="Completed story points against total active-sprint scope."
        >
          {sprintBurnup.length > 0 && sprintBurnup.some((item) => item.done_points > 0 || item.scope_points > 0) ? (
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={sprintBurnup}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(148, 163, 184, 0.15)" />
                  <XAxis
                    dataKey="date"
                    stroke="#94a3b8"
            ...
            <EmptyChart message="No active sprint burnup data available." />
          )}
        </ChartCard>
        ...
        <ChartCard
          title="WIP history"
          description="In Progress + Review workload compared with the combined WIP limit."
        >
          {wipHistory.length > 0 ? (
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={wipHistory}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(148, 163, 184, 0.15)" />
                  <XAxis
                    dataKey="date"
                    stroke="#94a3b8"
            ...
            <EmptyChart message="No WIP history available yet." />
          )}
        </ChartCard>

        <ChartCard
          title="Current WIP limits"
          description="Current workflow counts compared with configured WIP limits."
        >
          {wipLimits.length > 0 ? (
                ...
                >
                  <div className="mb-2 flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{formatStatus(item.status)}</p>
                    </div>
                    <Badge
                      className={cn(
                        "w-fit",
                  ...
                  <div className="flex items-end justify-between text-sm">
                    <span className="text-slate-400">Current cards</span>
            ...
            <EmptyChart message="No WIP configuration available." />
          )}
        </ChartCard>
      ...
      <div className="mt-8 rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
        ...
        <p className="text-sm leading-6 text-slate-400">
          Velocity uses closed sprints and completed story points. Burndown, burnup, cumulative flow and WIP history are reconstructed from sprint dates, task status and audit logs. Lead time is approximated from task creation to the first audit event where the task reached Done; flow lead time focuses on Kanban or non-sprint work.
        </p>
      </div>
    </div>
  );
