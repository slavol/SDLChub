...
def _can_manage_event(member, current_user: User, event: CalendarEvent) -> bool:
    role_name = member.role.name if member and member.role else "Member"
    return event.created_by_id == current_user.id or role_name in CALENDAR_MANAGEMENT_ROLES
...
def update_calendar_event(
    ...
    if not _can_manage_event(member, current_user, event):
...
def delete_calendar_event(
    ...
    member = check_project_permission(db, current_user.id, event.project_id)
    if not _can_manage_event(member, current_user, event):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You can only delete events you created.")
